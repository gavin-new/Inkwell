...
const i18n = {
  'zh-CN': {
    ...
    resetDocks: '复位所有工具栏到默认位置',
    docksReset: '已复位所有工具栏',
    assocBtn: '文件关联向导：注册并在系统弹窗中勾选 .md / .json 为默认打开方式',
    assocWizardDone: '已打开系统「设置程序关联」窗口：勾选 .md / .json 后点保存即设为默认',
    assocDone: '已加入「打开方式」列表；设为默认请在 系统设置 → 默认应用 中选择 Inkwell',
    assocFailed: '文件关联失败',
    bridgeUnavailable: '当前环境不支持此操作',
    jsonFormatBtn: '美化 JSON（格式化缩进）Ctrl+Shift+F',
    jsonPreviewHint: 'JSON 文档 · 预览为格式化 + 语法高亮',
    jsonInvalidTitle: 'JSON 解析失败',
    jsonFormatted: '已格式化 JSON',
    jsonAlreadyFormatted: 'JSON 已是格式化状态',
    // 数字
    wordCount: '{0} 字',
...
按 \`Ctrl+S\` 保存，按顶部"文件"菜单可导出为多种格式。祝写作愉快！`,
  ...
  'en': {
    ...
    resetDocks: 'Reset all toolbars to default position',
    docksReset: 'All toolbars reset',
    assocBtn: 'Association wizard: register and pick .md / .json as default in the system dialog',
    assocWizardDone: 'System "Set associations" window opened — tick .md / .json and save to set defaults',
    assocDone: 'Added to "Open with"; set as default via Settings → Default apps → Inkwell',
    assocFailed: 'File association failed',
    bridgeUnavailable: 'Not supported in this environment',
    jsonFormatBtn: 'Format JSON (pretty print) Ctrl+Shift+F',
    jsonPreviewHint: 'JSON document · preview is formatted & highlighted',
    jsonInvalidTitle: 'JSON parse failed',
    jsonFormatted: 'JSON formatted',
    jsonAlreadyFormatted: 'JSON already formatted',
    wordCount: '{0} chars',
